    ...
    public partial class orderday : Form
    {
       ...
       //SqlConnection con = new SqlConnection(@"Data Source=MOSA-PC\SQLEXPRESS;Initial Catalog=aserzamzam12;Integrated Security=True");
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            int n = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT ordid,KID, date, Kname, Knum, Kmark, date1, date2, priceG,FpriceG,SpriceG,kphone,priceK,tak FROM Korder WHERE convert(varchar,date)+convert(varchar,ordid)++convert(varchar,KID)+Kname+convert(varchar,Knum)+Kmark+convert(varchar,priceG)+convert(varchar,priceK)+convert(varchar,FpriceG)+convert(varchar,SpriceG)+convert(varchar,Kphone) LIKE '%" + textBox1.Text + "%'", con);
